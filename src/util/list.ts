...
export var _ = {
    ...
    treeFind<T>(list: T[], treeChildName: string = 'childs', predict: (t: T) => boolean): T {
        var result: T;
        var fc = function (arr) {
            for (var i = 0; i < arr.length; i++) {
                var item = arr[i];
                ...
                if (Array.isArray(item[treeChildName])) fc(item[treeChildName]);
                if (result) break;
            }
        }
        fc(list);
    ...
    treeFindAll<T>(list: T[], treeChildName: string = 'childs', predict: (t: T) => boolean): T[] {
        var results: T[] = [];
        var fc = function (arr) {
            for (var i = 0; i < arr.length; i++) {
                var item = arr[i];
